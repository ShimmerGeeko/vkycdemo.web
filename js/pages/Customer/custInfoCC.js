$('#btnNext').on('click', function () {
    ...
    if ($("form").valid()) {
        ...
        document.getElementById('rowBasicInfo1').style.display = 'none';
...
function previous() {
    ...
    document.getElementById('rowBasicInfo1').style.display = 'block';
}
...
ckCbsExCust.addEventListener('change', function () {
    if (this.checked) {
        $("#cbsCustId").prop('required', true);
        $("#cbsCustId").prop('disabled', false);
        $(function () {
            $.AdminBSB.input.activate();
        });
    }
    else {
        document.getElementById('cbsCustId').value = '';
        $("#cbsCustId").prop('required', false);
        $("#cbsCustId").prop('disabled', true);
        $(function () {
            $.AdminBSB.input.activate();
        });
        $('#cbsCustId').valid();
...
selCreditCard.addEventListener('change', function () {
    if(this.value.toUpperCase() === "Y"){
        document.getElementById('divBankName').style.display = 'block';
        document.getElementById('divCardLimit').style.display = 'block';
        $("#bCardIssuedBank").prop('required', true);
        $("#bCardLimit").prop('required', true);
        $(function () {
            $.AdminBSB.input.activate();
        });
    }
        ...
        $("#bCardIssuedBank").prop('required', false);
        $("#bCardLimit").prop('required', false);
        $(function () {
            $.AdminBSB.input.activate();
        });
    }
...
selSalaryAcc.addEventListener('change', function () {
    if(this.value.toUpperCase() === "OTHER"){
        document.getElementById('divNameOfBank').style.display = 'block';
        $("#bSalBankName").prop('required', true);
        $(function () {
            $.AdminBSB.input.activate();
        });
    }
    else{
        document.getElementById('divNameOfBank').style.display = 'none';
        $("#bSalBankName").prop('required', false);
        $(function () {
            $.AdminBSB.input.activate();
        });
    }
...
function loadDocFile() {
    var file = document.getElementById("bFile1").files[0];
...
function saveCustData(fileData, DocFile, filePasscode, xPanNumber) {
    try {
        ...
        var ExCustId = 0;
        var ExCustYN = 'N';
        var selExCust = document.getElementById('ckCbsExCust');
        if (selExCust.checked) {
            ExCustId = document.getElementById('cbsCustId').value;
            ExCustYN = 'Y';
        }
        var SalaryBankName;
        var selSalaryAcc = document.getElementById('selSalaryAcc');
        if (selSalaryAcc.value.toUpperCase() === 'OTHER') {
            SalaryBankName = document.getElementById('bSalBankName').value.toUpperCase();
        }
        else {
        ...
        var ExCCBankName = 'NA';
        var ExCCLimit = 'NA';
        var ExCCYN = document.getElementById('selCreditCard').value;
        if (ExCCYN === 'Y') {
            ExCCBankName = document.getElementById('bCardIssuedBank').value;
            ExCCLimit = document.getElementById('bCardLimit').value;
        }

        var CompanyName = document.getElementById('bCompanyName').value;
        ...
        var Data = {
            ...
            ExistingCustYN: ExCustYN,
            CbsCustNo: parseInt(ExCustId),
            Title: xTitle,
            ...
            CreditCardData: {
                ...
                ExistingCC: ExCCYN,
                ExBankName: ExCCBankName,
                ExCCLimit: ExCCLimit,
                SalaryAccBankName: SalaryBankName
            }
        };
        $.ajax({
            type: "POST",
